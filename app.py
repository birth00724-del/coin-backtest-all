...
st.set_page_config(page_title="TV-Style Supertrend + VWMA + VPVR-DVA Backtester (103-candle)", layout="wide")
st.title("📈 Supertrend(TradingView) 3중 결합 + VWMA 필터 + VPVR DVA — 103봉 롤링 / 프리셋 저장·불러오기 + 디버그 탭")
...
if uploaded:
    ...
    # ---------------- Tabs: 백테스트 / VPVR 디버그 ----------------
    tab1, tab2 = st.tabs(["🧪 전략 백테스트 (103 VPVR)", "🔎 VPVR 디버그(전체기간 롤링)"])
    ...
    with tab1:
        ...
        # ▶ 디버그 탭에서 펜딩한 bins가 있으면, 위젯 생성 전에 주입
        ...
        use_vpvr = st.sidebar.checkbox("VPVR DVA 필터 사용 (창: 고정 103일)", value=st.session_state.get("use_vpvr", False), key="use_vpvr")
        ...
        preset_name = c1.text_input("프리셋 이름", placeholder="예: STx3_VWMA20_VPVR103", key="preset_name")
        ...
        if not use_st and (use_vwma or use_vpvr):
            ...
            need_len = max(int(ST1_L), int(ST2_L), int(ST3_L), 103 if use_vpvr else 2, int(VWMA_L) if use_vwma else 2)
            if len(data) < need_len + 10:
                ...
                if st.button("🚀 백테스트 실행"):
                    if (use_vwma or use_vpvr) and "Volume" not in data.columns:
                        ...
                        st.subheader("📈 가격 & Supertrend (TV) " + ("+ VWMA" if use_vwma else "") + (" + VPVR DVA(103일)" if use_vpvr else ""))
                        fig = go.Figure()
                        ...
                        if use_vpvr and (dval_s is not None) and (dvah_s is not None):
                            fig.add_trace(go.Scatter(x=dval_s.index, y=dval_s.values, mode="lines", name="DVAL(103d)", line=dict(width=1, color="#455a64", dash="dash")))
                            fig.add_trace(go.Scatter(x=dvah_s.index, y=dvah_s.values, mode="lines", name="DVAH(103d)", line=dict(width=2, color="#1e88e5")))
    ...
    # =================== 탭2: VPVR 디버그 (전체 기간 롤링) ===================
    with tab2:
        st.markdown("### 🔎 VPVR 롤링 디버그 — **전체 기간**에 대해 매일 103봉 기준으로 DVAL/DVAH 재계산")
        if "Volume" not in data.columns:
            ...
            run_dbg = st.button("🧮 전체 기간 롤링 VPVR 계산 (103봉)")
            if run_dbg:
                with st.spinner("VPVR(103봉 롤링) 계산 중..."):
                    dval_s, dvah_s = compute_vpvr_dva(data, window=103, bins=int(bins))

                # Long 판단 (해당 일의 dvah가 존재하는 구간부터)
                close = data["Close"].astype(float)
                long_sig = (close > dvah_s)
                long_sig = long_sig & dvah_s.notna()

                # 전환점(매수/매도 발생일)
                long_shift = long_sig.shift(1, fill_value=False)
                buy_days  = data.index[(~long_shift) & ( long_sig)].strftime("%Y-%m-%d").tolist()
                sell_days = data.index[( long_shift) & (~long_sig)].strftime("%Y-%m-%d").tolist()

                st.success(f"계산 완료 (bins={int(bins)}, 윈도우=103봉). 매수 {len(buy_days)}건 / 매도 {len(sell_days)}건")
                c1, c2 = st.columns(2)
                with c1:
                    st.subheader("🟢 매수 발생일")
                ...
                # 라인 확인용 차트
                st.subheader("📈 Close vs DVAL/DVAH (103봉 롤링)")
                fig = go.Figure()
                fig.add_trace(go.Scatter(x=data.index, y=close, mode="lines", name="Close"))
                fig.add_trace(go.Scatter(x=dval_s.index, y=dval_s.values, mode="lines", name="DVAL(103)", line=dict(width=1, dash="dash")))
                fig.add_trace(go.Scatter(x=dvah_s.index, y=dvah_s.values, mode="lines", name="DVAH(103)", line=dict(width=2)))
                fig.update_layout(template="plotly_white", xaxis_title=("date_kst" if "date_kst" in cols_lower else "date_utc"), yaxis_title="Price")
                st.plotly_chart(fig, use_container_width=True)
                ...
                st.download_button(
                    ...
                    file_name=f"vpvr_rolling103_bins{int(bins)}.csv",
                    mime="text/csv"
                )
